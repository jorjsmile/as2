...
class AS2Adapter {
    ...
    public function __construct($partner_from, $partner_to, $as2BinDir = null){
        try {
            $this->partner_from = AS2Partner::getPartner($partner_from);
        }
        catch(\Exception $e){
            throw new AS2Exception('Sender AS2 id "' . $partner_from[0] . '" is unknown.');
        }
        ...
        try {
            ...
            throw new AS2Exception('Receiver AS2 id "' . $partner_to[0] . '" is unknown.');
        }

        $this->as2BinDir = self::resolveAs2BinDir($as2BinDir);
    }

    private static function resolveAs2BinDir($as2BinDir = null)
    {
        return $as2BinDir ?? __DIR__."/../bin/";
    }
    ...
    public function compose($files) {
        try {
            ...
            $command = self::$javapath . ' -jar '   . escapeshellarg($this->as2BinDir  . self::$ssl_adapter) .
                                         ' compose' .
                                         $args      .
...
//                                       ' -cypher '.escapeshellarg($cypher).
    ...
    {
        try {
            $command = self::$javapath . ' -jar '    . escapeshellarg(self::resolveAs2BinDir()  . self::$ssl_adapter) .
                                         ' checksum' .
                                         ' -in '     . escapeshellarg($input) . 
                                         ' 2>/dev/null';
    ...
    public static function getTempFilename(){
        if (is_null(self::$tmp_files)){
            ...
            register_shutdown_function(array(self::class, '_deleteTempFiles'));
        }
