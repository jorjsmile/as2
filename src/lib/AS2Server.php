...
class AS2Server {
    ...
    public static function handle($request = null)
    {
        ...
        try {
            ...
            $error = $e;
            AS2Log::error($e->getCode(), $e->getMessage());
        }
     ...
     * 
     * @param string $content       The content to save (mandatory)
     * @param array|\AS2Header $headers       The headers to save (optional)
     * @param string $filename      The filename to use if known (optional)
     * @param string $type    Values : raw | decrypted | payload (mandatory)
     * 
    ...
    protected static function saveMessage($content, $headers, $filename = '', $type = 'raw'){
        ...
        $dir = self::$messageDir . '_rawincoming';
        $dir = realpath($dir);
        @mkdir($dir, 0777, true);
