...
class MIME {
    ...
    /**
     * Internal recursive public static function to RFC 2047 encode a string.
     *
     * @access private
     *
     * @param string $text     The text to encode.
    ...
    public static function quotedPrintableEncode($text, $eol)
    {
        ...
        for ($i = 0; $i < $length; ++$i) {
            ...
            if ((($ascii === 32) &&
                ...
                $char_len = 3;
                $char = '=' . Str::upper(sprintf('%02s', dechex($ascii)));
            }
    ...
    public static function encodeAddress($addresses, $charset = null, $defserver = null)
    {
        if (is_array($addresses)) {
            ...
            $addr_arr = Parse::getInstance()->parse($addresses,true);
        }
    ...
    public static function decode($string, $to_charset = null)
    {
        ...
        case 'q':
            ...
            $decoded = Str::convertCharset($decoded, $charset, $to_charset);
            break;
    ...
    public static function encodeRFC2231($name, $string, $charset, $lang = null)
    {
        ...
        if (MIME::is8bit($string, $charset)) {
            $string = Str::lower($charset) . '\'' . (($lang === null) ? '' : Str::lower($lang)) . '\'' . rawurlencode($string);
            $encode = true;
        }
    ...
    public static function decodeRFC2231($string, $to_charset = null)
    {
        ...
        if (!is_null($charset)) {
            $output = Str::convertCharset($output, $charset, $to_charset);
        }
     ...
     */
    public static function trimEmailAddress($address)
    {
     ...
     */
    public static function rfc822WriteAddress($mailbox, $host = null, $personal = '')
    {
        $address = '';
     ...
     */
    public static function rfc822Explode($string, $delimiters = ',')
    {
        $emails = array();
        $pos = 0;
     ...
     */
    public static function addrObject2String($ob, $filter = '')
    {
     ...
     */
    public static function addrArray2String($addresses, $filter = '')
    {
        $addrList = array();
        ...
        foreach ($addresses as $addr) {
            ...
            if (!empty($val)) {
                $bareAddr = Str::lower(MIME::bareAddress($val));
     ...
     *                these addresses.
     */
    public static function bareAddress($address, $defserver = null, $multiple = false)
    {
        $addressList = array();
     ...
     */
    public static function parseAddressList($address, $defserver = null, $validate = false)
    {
        if (preg_match('/undisclosed-recipients:\s*;/i', trim($address))) {
            return array();
        }

        /* Use built-in IMAP public static function only if available and if not parsing
        ...
        if (!$validate &&
            ...
            $ret =
                $addr_arr = Parse::getInstance()->parse($address, false);
//                $parser->parseAddressList($address, $defserver, false,
//                                             $validate);
            return $ret;
        }
    }
     ...
     */
    protected static function _rfc822Encode($str, $type = 'address')
    {
