 ...
 *
 * The Structure:: class provides methods for dealing with MIME mail.
 *
 * The default character set to use for messages should be defined in the
 * variable $GLOBALS['Structure']['default_charset'] (defaults to US-ASCII
 * per RFC 2045).
 *
 * TODO: Convert to OO
 *
...
class Structure {
     ...
     *
     * @return &Message  The message parsed into a Message object.
     */
    public static function &parse($body)
    {
        ...
        $ptr = array(&$msgOb);
        Structure::addMultipartInfo($ptr);
    ...
    protected static function &_parse($body, $ref = 0)
    {
        ...
        if (isset($body->parts)) {
            ...
            while (list(,$sub_part) = each($body->parts)) {
                ...
                $Part->addPart(Structure::_parse($sub_part, $sub_ref), $sub_id++);
            }
        }
    ...
    protected static function _setInfo($part, &$ob, $ref)
    {
        ...
        foreach (Structure::_getParameters($part, 1) as $key => $val) {
            if ($key == 'charset') {
                $ob->setCharset($val);
            } else {
        ...
        /*if (($ob->getPrimaryType() == 'text') &&
            (Str::lower($ob->getCharset()) == 'us-ascii') &&
            isset($GLOBALS['Structure']['default_charset'])) {
            $ob->setCharset($GLOBALS['Structure']['default_charset']);
        }*/

        /* Go through the disposition parameters, if any. */
        foreach (Structure::_getParameters($part, 2) as $key => $val) {
    ...
    public static function addMultipartInfo(&$parts, $info = array())
    {
        ...
        while (list($key,) = each($parts)) {
            ...
            Structure::addMultipartInfo($ptr->getParts(), $new_info);
        }
    }

    /**
     * Attempts to build a Message object from a text message.
     *
     * @param string $text  The text of the MIME message.
     *
     * @return Message  A Message object, or false on error.
     */
    ...
    protected static function _convertMimeDecodeData(&$ob)
    {
        ...
        if (isset($ob->ctype_secondary)) {
            $ob->subtype = Str::upper($ob->ctype_secondary);
            $ob->ifsubtype = 1;
        } else {
            $ob->ifsubtype = 0;
        }
        ...
        if (isset($ob->parts)) {
            ...
            while (list($key,) = each($ob->parts)) {
                Structure::_convertMimeDecodeData($ob->parts[$key]);
            }
        }
    }
    ...
    public static function parseMIMEHeaders($headers, $decode = true, $lowercase = false)
    {
        ...
        foreach ($toprocess as $val) {
            if ($decode) {
                ...
                if (in_array(Str::lower($val[0]), array('from', 'to', 'cc', 'bcc', 'reply-to', 'resent-to', 'resent-cc', 'resent-bcc', 'resent-from', 'sender'))) {
