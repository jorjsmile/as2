...
use Jorjsmile\AS2\Helpers\Str;
use Jorjsmile\AS2\Helpers\Util;

/**
 * The Part:: class provides a wrapper around MIME parts and methods
...
class Part {
     ...
     */
    const MIME_DEFAULT_CHARSET = 'us-ascii';


    /**
     * The default MIME description.
     */
    const MIME_DEFAULT_DESCRIPTION = 'unnamed';

    /**
     * The default MIME disposition.
     */
    const MIME_DEFAULT_DISPOSITION = 'inline';

    /**
     * The default MIME encoding.
     */
    const MIME_DEFAULT_ENCODING = '7bit';
     ...
     * @var string
     */
    protected $_transferEncoding = self::MIME_DEFAULT_ENCODING;
     ...
     * @var string
     */
    protected $_disposition = self::MIME_DEFAULT_DISPOSITION;
    ...
    public function __construct($mimetype = null, $contents = null,
                                $charset = self::MIME_DEFAULT_CHARSET, $disposition = null,
                                $encoding = null)
    {
        /* Create the unique Part boundary string. */
    ...
    {
        $disposition = Str::lower($disposition);
    ...
    {
        return $this->replaceEOL($this->_contents, self::Part_RFC_EOL);
    }
    ...
    public function setType($mimetype)
    {
        ...
        list($this->_type, $this->_subtype) = explode('/', Str::lower($mimetype));
     ...
     * Add a MIME subpart.
     *
     * @param Part $Part  Add a Part subpart to the current
     *                              Part.
     * @param string $index         The index of the added Part.
     */
    public function addPart($Part, $index = null)
    {
     ...
     * @param Part $Part  The MIME Part to store.
     */
    public function alterPart($id, $Part)
    {
        if (($ptr = &$this->_partFind($id, $this->_parts))) {
            $ptr = $Part;
            $this->_idmap = array();
     ...
     *
     * @param string $id         The Part ID string.
     * @param array &$parts      A list of Part objects.
    ...
    public function &_partFind($id, &$parts, $retarray = false)
    {
         ...
         * of Parts or an array of arrays. */
        $check = reset($this->_idmap);
        if (empty($check) || !is_a($check, 'Part')) {
            $this->_idmap = array();
    ...
    /**
     * Generates a mapping of Parts with their MIME IDs.
     *
     * @access private
     *
     * @param array &$parts  An array of Parts to map.
     */
    ...
    /**
     * Add information about the Part.
     *
    ...
    /**
     * Retrieve information about the Part.
     *
     * @param string $label  The information label.
     *
    ...
    public function toCanonicalString($headers = true)
    {
        ...
        return $this->replaceEOL($string, self::Part_RFC_EOL);
    }
    ...
    /**
     * Return the unique Part boundary string generated for this object.
    ...
    /**
     * Determine the size of a Part and its child members.
     *
    ...
    public function getBytes()
    {
        ...
        if (empty($this->_flags['contentsSet']) && $this->_bytes) {
            ...
            if ($this->getPrimaryType() == 'text') {
                $bytes = Str::length($this->_contents, $this->getCharset());
            } else {
     ...
     * contents of the part are not fully loaded (i.e. creating a Part
