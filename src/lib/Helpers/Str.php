...
namespace Jorjsmile\AS2\Helpers;
/**
 * The Str:: class provides static methods for charset and locale safe
 * string manipulation.
 *
 * $Horde: framework/Util/Str.php,v 1.43.6.37 2009/03/30 15:31:38 jan Exp $
 *
...
class Str {
    ...
    /**
     * Sets a default charset that the Str:: methods will use if none is
    ...
    public static function setDefaultCharset($charset)
    {
        ...
        if (Str::extensionExists('mbstring') &&
            function_exists('mb_regex_encoding')) {
            $old_error = error_reporting(0);
            mb_regex_encoding(Str::_mbstringCharset($charset));
    ...
    public static function convertCharset($input, $from, $to = null)
    {
        ...
        if (is_array($input)) {
            ...
            while (list($key, $val) = each($input)) {
                $tmp[Str::_convertCharset($key, $from, $to)] = Str::convertCharset($val, $from, $to);
            }
            return $tmp;
        }
        if (is_object($input)) {
            ...
            while (list($key, $val) = each($vars)) {
                $input->$key = Str::convertCharset($val, $from, $to);
            }
            return $input;
        }
        ...
        return Str::_convertCharset($input, $from, $to);
    }
     ...
     * @access private
     *
     * @param string $input  See Str::convertCharset().
     * @param string $from   See Str::convertCharset().
     * @param string $to     See Str::convertCharset().
     *
    ...
    protected static function _convertCharset($input, $from, $to)
    {
        ...
        if (strlen($input) < 16777216 || !(Str::extensionExists('iconv') || Str::extensionExists('mbstring'))) {
        ...
        if (($from != 'utf7-imap') &&
            ...
            $output = (isset($php_errormsg)) ? false : Str::substr($output, 0, -1, $to);
            ini_set('track_errors', $oldTrackErrors);
        }

        /* Next try mbstring. */
        if (!$output && Str::extensionExists('mbstring')) {
            $old_error = error_reporting(0);
            $output = mb_convert_encoding($input, $to, Str::_mbstringCharset($from));
            error_reporting($old_error);
        }

        /* At last try imap_utf7_[en|de]code if appropriate. */
        if (!$output && Str::extensionExists('imap')) {
    ...
    public static function lower($string, $locale = false, $charset = null)
    {
        ...
        if ($locale) {
            ...
            if (Str::extensionExists('mbstring') &&
                ...
                $old_error = error_reporting(0);
                $ret = mb_strtolower($string, Str::_mbstringCharset($charset));
                error_reporting($old_error);
                if (!empty($ret)) {
                    return $ret;
                }
    ...
    public static function upper($string, $locale = false, $charset = null)
    {
        ...
        if ($locale) {
            ...
            if (function_exists('mb_strtoupper')) {
                ...
                $old_error = error_reporting(0);
                $ret = mb_strtoupper($string, Str::_mbstringCharset($charset));
                error_reporting($old_error);
                if (!empty($ret)) {
                    return $ret;
                }
    ...
    {
        if ($locale) {
            $first = Str::substr($string, 0, 1, $charset);
            if (Str::isAlpha($first, $charset)) {
                $string = Str::upper($first, true, $charset) . Str::substr($string, 1, null, $charset);
            }
        } else {
            $string = Str::upper(substr($string, 0, 1), false) . substr($string, 1);
        }
        return $string;
    }
    ...
    {
        if (is_null($length)) {
            $length = Str::length($string, $charset) - $start;
        }
        ...
        if (Str::extensionExists('mbstring')) {
            ...
            $old_error = error_reporting(0);
            $ret = mb_substr($string, $start, $length, Str::_mbstringCharset($charset));
            error_reporting($old_error);
    ...
    public static function length($string, $charset = null)
    {
        ...
        if (Str::extensionExists('mbstring')) {
            $old_error = error_reporting(0);
            $ret = mb_strlen($string, Str::_mbstringCharset($charset));
            error_reporting($old_error);
            if (!empty($ret)) {
                return $ret;
            }
    ...
    public static function pos($haystack, $needle, $offset = 0, $charset = null)
    {
        if (Str::extensionExists('mbstring')) {
            ...
            $old_error = error_reporting(0);
            $ret = mb_strpos($haystack, $needle, $offset, Str::_mbstringCharset($charset));
            error_reporting($old_error);
    ...
    public static function pad($input, $length, $pad = ' ', $type = STR_PAD_RIGHT,
                 $charset = null)
    {
        $mb_length = Str::length($input, $charset);
        $sb_length = strlen($input);
        $pad_length = Str::length($pad, $charset);
        ...
        case STR_PAD_LEFT:
            ...
            $output = Str::substr(str_repeat($pad, ceil($left / $pad_length)), 0, $left, $charset) . $input;
            break;
        case STR_PAD_BOTH:
            $left = floor(($length - $mb_length) / 2);
            $right = ceil(($length - $mb_length) / 2);
            $output = Str::substr(str_repeat($pad, ceil($left / $pad_length)), 0, $left, $charset) .
                $input .
                Str::substr(str_repeat($pad, ceil($right / $pad_length)), 0, $right, $charset);
            break;
        case STR_PAD_RIGHT:
            $right = $length - $mb_length;
            $output = $input . Str::substr(str_repeat($pad, ceil($right / $pad_length)), 0, $right, $charset);
            break;
        }
     ...
     * @since Horde 3.2
     *
     * @param string $string         Str containing the text to wrap.
     ...
     *
     * @return string  Str containing the wrapped text.
     */
                      ...
                      $charset = null, $line_folding = false)
    {
        ...
        $charset = Str::_mbstringCharset($charset);
        $string = Str::convertCharset($string, $charset, 'utf-8');
        $wrapped = '';

        while (Str::length($string, 'utf-8') > $width) {
            $line = Str::substr($string, 0, $width, 'utf-8');
            $string = Str::substr($string, Str::length($line, 'utf-8'), null, 'utf-8');
            // Make sure didn't cut a word, unless we want hard breaks anyway.
            if (!$cut && preg_match('/^(.+?)(\s|\r?\n)/u', $string, $match)) {
                $line .= $match[1];
                $string = Str::substr($string, Str::length($match[1], 'utf-8'), null, 'utf-8');
            }
            ...
            if ($cut) {
                $wrapped .= Str::substr($line, 0, $width, 'utf-8') . $break;
                $string = Str::substr($line, $width, null, 'utf-8') . $string;
                continue;
            }
            $wrapped .= $line;
        }

        return Str::convertCharset($wrapped . $string, 'utf-8', $charset);
    }

    /**
     * Wraps the text of a message.
     *
     * @param string $text        Str containing the text to wrap.
                  ...
                  $quote = false)
    {
        ...
        foreach (preg_split('/\r?\n/', $text) as $input) {
            if ($quote && (strpos($input, '>') === 0)) {
                ...
                $line = Str::wordwrap($input, $length, $break_char, false, $charset);
            }
    ...
    {
        return ((Str::lower($string, true, $charset) === $string) &&
                Str::isAlpha($string, $charset));
    }
    ...
    {
        return ((Str::upper($string, true, $charset) === $string) &&
                Str::isAlpha($string, $charset));
    }
    ...
    {
        if (!empty($charset)) {
            $regex = Str::convertCharset($regex, $charset, 'utf-8');
            $text = Str::convertCharset($text, $charset, 'utf-8');
        }
        ...
        if (!empty($charset)) {
            $matches = Str::convertCharset($matches, 'utf-8', $charset);
        }
    ...
    protected static function _mbstringCharset($charset)
    {
        ...
        if (in_array(Str::lower($charset), array('ks_c_5601-1987', 'ks_c_5601-1989'))) {
            $charset = 'UHC';
        }
